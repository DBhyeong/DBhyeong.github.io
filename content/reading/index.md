---
title: 읽을거리 (지식 가든)
description: "AI 에이전트·코딩·하네스·LLM 지식위키·GEO 등 외부 자료를 적대적 팩트체크와 함께 정리한 Hyeong의 지식 가든."
tags:
  - reading
---

논문·긱뉴스·블로그·강연·GitHub repo 등 **외부에서 읽은 자료를 정리한 노트**입니다. 블로그의 과장(스타 수·성능 단정·라이선스 등)은 **1차 출처로 팩트체크**해 정정합니다. 그래프뷰·태그·검색으로 탐색하세요.

## 🤖 AI 에이전트 · 코딩 · 하네스 · 루프
- [[ai-agent-coding-document-ai-6-sources-deep-dive|AI 에이전트 시대 개발·데이터 워크플로 6선 심층분석]]
- [[ponytail-lazy-senior-dev-skill|Ponytail — '게으른 시니어' AI 코드 최소화 스킬 (팩트체크)]]
- [[harness-engineering-claude-code-design-guide|Harness Engineering — Claude Code 설계 가이드]]
- [[harness-design-claude-code-vs-codex-book-two|하네스 설계 — Claude Code vs Codex]]
- [[harness-engineering-checklist|하네스 엔지니어링 실무 체크리스트]]
- [[awesome-harness-engineering-curated-list|Awesome Harness Engineering — 자료 모음 (팩트체크)]]
- [[agent-skills-addy-osmani|Agent Skills (Addy Osmani)]]
- [[loop-engineering-addy-osmani|Loop Engineering (Addy Osmani)]]
- [[designing-loops-with-fable-5-anthropic|Fable 5 루프 설계 (Anthropic)]]
- [[designing-agentic-loops-simon-willison|에이전트 루프 설계 (Simon Willison)]]
- [[compound-engineering-every-kieran-klaassen|Compound Engineering (Every)]]
- [[anthropic-claude-code-workflow-demo|Anthropic Claude Code 워크플로 데모]]
- [[claude-code-productivity-neil-kakkar|Claude Code 생산성 (Neil Kakkar)]]
- [[loop-engineering-llm-data-engineering-book-to-skill|루프 엔지니어링 · LLM 데이터 엔지니어링 · Book-to-Skill]]

## 📚 LLM 지식위키 · 지식관리
- [[karpathy-llm-knowledge-bases|Karpathy LLM 지식베이스 원문 정리]]
- [[llm-wiki-types-taxonomy|LLM Wiki 종류 · 계보 정리]]
- [[llm-wiki-three-repos-and-precise-citation-kb|LLM 지식위키 3대 저장소 + 정밀 인용 KB]]
- [[llm-wiki-agent-knowledge-base-ecosystem|LLM 위키 생태계 (긱뉴스)]]
- [[knowledge-tooling-ecosystem-llm-wiki-obsidian-skills|지식관리 도구 생태계]]
- [[folder-native-ai-agent-okf-icm|폴더 네이티브 AI 에이전트 — OKF + ICM]]
- [[claude-obsidian-agricidaniel|claude-obsidian (Karpathy 패턴 플러그인)]]
- [[llm-graph-builder-neo4j-knowledge-graph|LLM Graph Builder — 비정형 데이터 → Neo4j 지식그래프 (팩트체크)]]

## 🔍 GEO · 마케팅 · 뉴스
- [[geoflow-geo-content-engine|GEOFlow — GEO 콘텐츠 엔진]]
- [[llm-ai-news-sources-catalog|LLM/AI 정보원 · 구독처 카탈로그]]
- [[korea-ai-news-agent-governance-2026-06-23|국내 AI 이슈 — 에이전트 거버넌스 外 (AI타임스)]]
- [[ai-it-digest-2026-06-24|AI/IT 데일리 다이제스트 (2026-06-24)]]

---
*외부 공개 자료 정리 노트. 1차 출처 팩트체크 반영. 사적·내부 자료는 제외.*
