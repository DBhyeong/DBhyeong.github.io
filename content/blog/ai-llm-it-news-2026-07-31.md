...
|---|---|
| 다지 손 — 전구 **푸는** 동작 | **92퍼센트** |
| 다지 손 — 전구 **조이는** 동작 | **36퍼센트** |
| 테이블에서 집기 | 68.4퍼센트 |
| 선반에서 집기 | 76.3퍼센트 |
| 바닥에서 집기 | 45.7퍼센트 |
| 정밀 삽입 (그리퍼) | 89.6퍼센트 |

같은 손, 같은 물체, 같은 나사산이다. **회전 방향만 반대인데 성공률이 절반 이하로 떨어진다.** 그리고 집기 과제도 표면 높이에 따라 45.7에서 76.3까지 벌어진다.

이 표를 "전구 교체 과제 평균 64퍼센트"로 요약하면 정보가 통째로 사라진다. **평균이 지우는 것이 여기서는 실패의 위치다.** 조이는 동작이 어려운 건 힘 제어와 나사산 정렬이 필요해서인데, 그 정보가 평균 한 줄에는 남지 않는다. 로봇 벤치마크를 읽을 때는 항목별 분해가 있는지부터 봐야 한다는 게 이 표의 교훈이다.
...
| Gemini Robotics 성공률 | ✅ 블로그 차트 기준. 항목별 편차가 커서 평균 요약 부적합 |
